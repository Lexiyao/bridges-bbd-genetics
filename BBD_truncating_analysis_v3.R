...
OUT_XLSX   <- "BBD_truncating_results_v3.xlsx"
...
dat_bbd_all <- dat_status0 %>%
  filter(!is.na(BBD_history), !is.na(ageInt)) %>%
...
make_forest_plot <- function(res_df, title_text, filename) {
  ...
  x_max <- max(c(plot_df$CI_high, 5.0), na.rm = TRUE) * 1.20
...
si <- sessionInfo()
writeLines(capture.output(si), "session_info_truncating.txt")
message("Session info saved to: session_info_truncating.txt")
