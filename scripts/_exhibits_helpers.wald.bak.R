...
CAPTION_TRUNCATING <- paste(
  "Firth's penalised logistic regression (Wald CIs). Adjusted for age and study.",
  "BH-FDR across 9 genes. Red = FDR q < 0.05; arrow = CI truncated at panel edge.",
  sep = "\n")

CAPTION_MISSENSE <- paste(
  "Firth's penalised logistic regression (Wald CIs). Carriers: CADD phred ≥ 20 (top 1% deleterious).",
...
forest_plot <- function(df, title, file,
  ...
  p <- ggplot(plot_df, aes(y = gene_f)) +
    ...
    labs(title = title, x = "Odds Ratio (log scale, 95% Wald CI)", y = NULL,
         caption = caption) +
...
or_heatmap <- function(df, file,
                       ...
                       subtitle = "* = FDR q < 0.05 (bold). NE = not estimable (no carriers among cases). Colour capped at OR 0.25–8.",
                       genes      = GENES_PANEL,
                       col_levels = NULL,
                       width = 9, height = 6) {
  ...
  hm <- df %>%
    mutate(gene = factor(gene, levels = rev(genes)),
           ...
           lab  = ifelse(is.na(OR), "NE", sprintf("%.2f%s", OR, ifelse(bold, "*", ""))))

  p <- ggplot(hm, aes(col, gene, fill = l2)) +
    geom_tile(colour = "white", linewidth = 1.1) +
    geom_text(aes(label = lab, fontface = ifelse(bold, "bold", "plain")), size = 3.4) +
